...
export interface IDefaultRoyaltyButtonProps {
  className?: string;
  contract: IContract;
  disabled?: boolean;
...
export const DefaultRoyaltyButton: FC<IDefaultRoyaltyButtonProps> = props => {
  const { className, contract, disabled, variant } = props;

  const { address, contractFeatures, royalty, contractType } = contract;
  ...
  return (
    <Fragment>
      <ListAction
        ...
        className={className}
        dataTestId="DefaultRoyaltyButton"
        disabled={
          disabled ||
