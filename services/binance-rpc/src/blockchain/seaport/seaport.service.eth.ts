...
export class SeaportServiceEth {
  ...
  public async orderCancelled(event: ILogEvent<ISeaportOrderFulfilled>, context: Log): Promise<void> {
    await this.updateHistory(event, context);
  }

  public async orderValidated(event: ILogEvent<ISeaportOrderFulfilled>, context: Log): Promise<void> {
    await this.updateHistory(event, context);
  }

  public async nonceIncremented(event: ILogEvent<ISeaportOrderFulfilled>, context: Log): Promise<void> {
    await this.updateHistory(event, context);
  }
