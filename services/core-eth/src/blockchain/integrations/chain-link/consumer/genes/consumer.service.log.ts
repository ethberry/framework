...
import { ERC721GenesABI } from "./interfaces";
...
export class ChainLinkConsumerServiceLog {
  ...
  public async initRegistry(): Promise<void> {
    ...
    const contractEntities = await this.contractService.findAll({
      contractFeatures: In([[ContractFeatures.GENES]]),
      chainId,
    });
  ...
  public updateRegistry(address: Array<string>): void {
    this.ethersService.updateRegistry({
      contractType: ContractType.VRF_GENES,
      contractAddress: address,
      contractInterface: ERC721GenesABI,
      eventSignatures: [ChainLinkEventSignature.MintGenes],
    });
  }
}
