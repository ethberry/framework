...
@Injectable()
export class LegacyVestingServiceEth {
  constructor(
    ...
    private readonly vestingServiceLog: LegacyVestingServiceLog,
  ) {}

  public async erc20Released(event: ILogEvent<ILegacyVestingERC20ReleasedEvent>, context: Log): Promise<void> {
    const {
      name,
      args: { token, amount },
    } = event;
  ...
  public async ethReleased(event: ILogEvent<ILegacyVestingEtherReleasedEvent>, context: Log): Promise<void> {
    const {
      name,
      args: { amount },
    } = event;
  ...
  public async ethReceived(event: ILogEvent<ILegacyVestingPaymentReceivedEvent>, context: Log): Promise<void> {
    const {
      name,
      args: { amount },
    } = event;
  ...
  public deploy(event: ILogEvent<IContractManagerLegacyVestingDeployedEvent>): void {
    const {
      args: { account },
    } = event;
