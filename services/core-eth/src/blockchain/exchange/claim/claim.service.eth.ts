...
export class ExchangeClaimServiceEth {
  ...
  public async claimTemplate(event: ILogEvent<IExchangeClaimTemplateEvent>, context: Log): Promise<void> {
    const {
      name,
    ...
    await this.notificatorService.claimTemplate({
      claim: claimEntity,
      address,
      transactionHash,
    });
  ...
  public async claimToken(event: ILogEvent<IExchangeClaimTokenEvent>, context: Log): Promise<void> {
    const {
      name,
      args: { account, items, externalId },
    } = event;
    ...
    await this.notificatorService.claimToken({
      claim: claimEntity,
      address,
      transactionHash,
    });
