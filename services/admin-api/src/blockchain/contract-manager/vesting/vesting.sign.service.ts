...
export class ContractManagerLegacyVestingSignService {
  ...
  public async deploy(dto: IVestingContractDeployDto, userEntity: UserEntity): Promise<IServerSignature> {
    const nonce = randomBytes(32);
    const { bytecode } = await this.getBytecodeByVestingContractTemplate(dto, userEntity.chainId);

    await this.planService.validateDeployment(userEntity, ModuleType.VESTING, TokenType.ERC721);
    ...
    const signature = await this.signer.signTypedData(
      ...
      // Values
      {
        ...
        args: {
          contractTemplate: Object.values(VestingContractTemplates).indexOf(dto.contractTemplate).toString(),
          name: dto.name,
          symbol: dto.symbol,
  ...
  public getBytecodeByVestingContractTemplate(dto: IVestingContractDeployDto, chainId: number) {
    ...
    switch (contractTemplate) {
      case VestingContractTemplates.SIMPLE:
        return getContractABI(
          "@framework/core-contracts/artifacts/contracts/Mechanics/Vesting/ERC721Vesting.sol/ERC721Vesting.json",
          chainId,
        );
      default:
