...
import { CraftERC1155, Resources } from "../../typechain-types";
...
describe("CraftERC1155", function () {
  ...
  let resourceInstance: Resources;
  ...
  beforeEach(async function () {
    ...
    resource = await ethers.getContractFactory("Resources");
    resourceInstance = (await resource.deploy(baseTokenURI)) as Resources;
  ...
  describe("hasRole", function () {
    it("Should set the right roles to deployer", async function () {
      const isAdmin = await refineryInstance.hasRole(DEFAULT_ADMIN_ROLE, owner.address);
      expect(isAdmin).to.equal(true);
      const isMinter = await refineryInstance.hasRole(PAUSER_ROLE, owner.address);
  ...
  describe("craft", function () {
    it("should craft resources lvl2", async function () {
      await resourceInstance.mint(receiver.address, tokenId, 10, "0x");

      await resourceInstance.connect(receiver).setApprovalForAll(refineryInstance.address, true);

      const tx1 = refineryInstance.connect(receiver).craft(1, 1);
      await expect(tx1)
        .to.emit(resourceInstance, "TransferBatch")
        .withArgs(refineryInstance.address, receiver.address, ethers.constants.AddressZero, [tokenId], [10])
        .emit(resourceInstance, "TransferSingle")
        .withArgs(refineryInstance.address, ethers.constants.AddressZero, receiver.address, 4, 1);

      const balance = await resourceInstance.balanceOf(receiver.address, tokenId);
      expect(balance).to.equal(0);
    });

    it("should fail: insufficient permissions", async function () {
      const tx = refineryInstance.connect(receiver).craft(1, 1);
    ...
    it("should fail: no such recipe", async function () {
      const tx = refineryInstance.connect(receiver).craft(2, 1);
  ...
  describe("updateRecipe", function () {
    ...
    it("should fail: for wrong role", async function () {
      const tx1 = refineryInstance.connect(receiver).updateRecipe(0, false);
