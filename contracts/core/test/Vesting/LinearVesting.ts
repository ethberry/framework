...
describe.skip("LinearVesting", function () {
  const span = 2500;
  ...
  beforeEach(async function () {
    ...
    vestingInstance = await vestingFactory.deploy(this.receiver.address, Math.ceil(Date.now() / 1000), 10000);
  ...
  it("should release", async function () {
    ...
    await time.advanceBlockTo(current1.add(web3.utils.toBN(span)));

    const tx2 = await vestingInstance["release()"]();
    await expect(tx2).to.changeEtherBalances(
      [vestingInstance, this.receiver],
      [constants.WeiPerEther.div(4).mul(-1), constants.WeiPerEther.div(4)],
    );

    const current2 = await time.latestBlock();
    await time.advanceBlockTo(current2.add(web3.utils.toBN(span)));
    ...
    const current3 = await time.latestBlock();
    await time.advanceBlockTo(current3.add(web3.utils.toBN(span)));
    ...
    const current4 = await time.latestBlock();
    await time.advanceBlockTo(current4.add(web3.utils.toBN(span)));
