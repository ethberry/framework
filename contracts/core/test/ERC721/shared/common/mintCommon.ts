...
export function shouldMintCommon() {
  describe("mintCommon", function () {
    ...
    it("should mint to receiver", async function () {
      const tx = this.contractInstance.mintCommon(this.erc721ReceiverInstance.address, templateId);
      await expect(tx)
        .to.emit(this.contractInstance, "Transfer")
        .withArgs(constants.AddressZero, this.erc721ReceiverInstance.address, tokenId);

      const balance = await this.contractInstance.balanceOf(this.erc721ReceiverInstance.address);
      expect(balance).to.equal(1);
    });

    it("should fail: wrong role", async function () {
      const tx = this.contractInstance.connect(this.receiver).mintCommon(this.receiver.address, templateId);
    ...
    it("should fail: to mint to non receiver", async function () {
      const tx = this.contractInstance.mintCommon(this.erc721NonReceiverInstance.address, templateId);
